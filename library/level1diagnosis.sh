...
TPCPU=$(ps aux --sort -%cpu | head -4 | awk 'BEGIN {ORS=","} NR>1{print "{\"ProcessID\":\""$2"\", \"CMD\":\""$11"\", \"CPUPercent\":\""$3"\"}"}')
TPMEM=$(ps aux --sort -%mem | head -4 | awk 'BEGIN {ORS=","} NR>1{print "{\"ProcessID\":\""$2"\", \"CMD\":\""$11"\", \"MemoryPercent\":\""$4"\"}"}')
