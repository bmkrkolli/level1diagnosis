...
if [ -z "$checkfilesystem" ]; then
    FS=$(df -TPh -x squashfs -x tmpfs -x devtmpfs | awk 'BEGIN {ORS=","} NR>1{print "{\"Mount\":\""$7"\", \"UsedPercent\":"${6::-1}" }"}'||echo 'df command not found,')
else
    CFS=$checkfilesystem
    FS=$(df -TPh $CFS | awk 'BEGIN {ORS=","} NR>1{print "{\"Mount\":\""$7"\", \"UsedPercent\":"${6::-1}" }"}'||echo 'df command not found,')
fi
